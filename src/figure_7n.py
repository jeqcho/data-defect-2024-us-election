...
# Modified from figure_7.py to use sample size instead of total votes for the x-axis
...
def plot_z_scores(data, candidate, ax):
    ...
    for _, row in data.iterrows():
        # Use sample size instead of total votes
        x_pos = math.log10(row["num_respondents_all"])
    ...
    adjust_text(
        texts,
        x=data["num_respondents_all"].apply(math.log10),
        y=data[z_score_col],
        ax=ax,
        expand=(4, 4),
    )
    ...
    # Calculate appropriate x-axis ticks based on sample size range
    min_x = math.floor(min(data["num_respondents_all"].apply(math.log10)))
    max_x = math.ceil(max(data["num_respondents_all"].apply(math.log10)))
    ticks = np.linspace(min_x, max_x, 5)
    ax.xaxis.set_major_locator(FixedLocator(ticks))
    # Format the x-axis tick labels
    ax.set_xticklabels([f"{tick:.1f}" for tick in ticks])
    # Set x-axis limits to match the ticks with a bit of padding
...
def main():
    ...
    merged_data[output_columns].to_csv("../data/figure_7n.csv", index=False)
    ...
    plt.tight_layout()
    plt.savefig("../figures/figure_7n.png", dpi=300)
